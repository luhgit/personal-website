...
export default function SearchCollection({ entry_name, data, tags }: Props) {
  // Use union type, not forced 'blog'
  const coerced: BlogOrProject[] = data

  const [query, setQuery] = createSignal("")
  const [filter, setFilter] = createSignal(new Set<string>())
  const [collection, setCollection] = createSignal<BlogOrProject[]>([])
  const [descending, setDescending] = createSignal(false)

  // Safer tag getter
  const getTags = (entry: BlogOrProject): string[] =>
    Array.isArray((entry.data as any).tags) ? ((entry.data as any).tags as string[]) : []
  ...
  createEffect(() => {
    const searchResult =
      query().length < 2 ? coerced : fuse.search(query()).map((r) => r.item)

    const filtered = searchResult.filter((entry) => {
      const entryTags = getTags(entry).map((t) => t.toLowerCase())
      const active = Array.from(filter())
      if (active.length === 0) return true
      // every selected tag must be present on the entry
      return active.every((t) => entryTags.includes(String(t).toLowerCase()))
    })
        ...
        <div class="flex flex-col">
          ...
          <ul class="flex flex-col gap-3">
            {collection().map((entry) => (
              <li>
                {/* pill=true will show "post/project" badge if you want it */}
